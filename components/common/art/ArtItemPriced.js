...
const ArtItemPriced = ({ nft, bidAvailable, ...props }) => {
  ...
  const { offer, removeSale, marketContract } = useContext(MarketContractContext);
  ...
  return (
    <ArtItem
      buttonText={isItemOwnedByUser() ? 'Unlist' : `Buy for ${getNextBidNearsFormatted(nft)}Ⓝ`}
